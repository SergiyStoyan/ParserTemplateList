    ...
    public partial class MainForm : Form
    {
        ...
        internal bool TryLaunchProcessorThread(string progressTask, Func<bool> preProcessorCode, MethodInvoker processorCode/*, Win.ThreadRoutines.ErrorHandler exceptionCode=null*/, MethodInvoker finallyCode = null)
        {
            ...
            processorThread = Win.ThreadRoutines.StartTry(
                ...
                {
                    Log.Error(e);
                    This.SetProgressTask("ERROR!", Color.Red);
                    ThreadRoutines.Start(() => { Message.Error(e, this); });
                },
                () =>
                {
                    finallyCode?.Invoke();
        ...
        internal bool StopPpocessorThread()
        {
            ...
            if (processorThread.TryAbort(1000))
            {
                ...
                This.SetProgressTask("TERMINATED", Color.Yellow);
                ThreadRoutines.Start(() => { Message.Inform("TERMINATED..."); });
                PpocessorThreadStateChange?.BeginInvoke(false, null, null);
            }
